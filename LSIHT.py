...
class LeastSquaresIHT(ERCcheckMixin, SparseSolver):
    ...
    def __str__(self):
        return "LSIHT ("+str(self.mu)+","+str(self.stopval)+")"

    __repr__ = __str__

    def solve(self, data, dictionary, realdict=None):
        k = realdict['support'].shape[0]
        return _least_squares_IHT(data, dictionary, k, self.mu, self.stopval)
...
def _least_squares_IHT(data, dictionary, k, mu, stopval):
    """
    ...
    Nullsp = Vt[n:]     # last N-n rows of Vt, on rows
